...
# allow duplicate meta for build+plan orchestrator + build-worker (all same meta model) — council needs strong reasoning
dups_agents=$(jq -r '.agent[]?.model // empty' opencode.json.example | sort | uniq -d)
# only meta duplicate is allowed (build + plan + build-worker all meta)
...
[[ "$total_entries" -eq 9 ]] || fail "expected 9 model entries (model+small+7 agents), got $total_entries"
...
jq -e '.agent."build-worker".model=="meta/muse-spark-1.2-contributor"' opencode.json.example >/dev/null || fail "build-worker should use meta (same as build)"
...
jq -e '.agent."build-worker"' opencode.json.example >/dev/null || fail "build-worker agent missing"
